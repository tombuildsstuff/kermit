...
// WorkspaceGitRepoManagementClient is the client for the WorkspaceGitRepoManagement methods of the Synapse service.
...
func (client WorkspaceGitRepoManagementClient) GetGitHubAccessToken(ctx context.Context, APIVersion string, gitHubAccessTokenRequest GitHubAccessTokenRequest, clientRequestID string) (result GitHubAccessTokenResponse, err error) {
    if tracing.IsEnabled() {
        ...
        return result, validation.NewError("synapse.WorkspaceGitRepoManagementClient", "GetGitHubAccessToken", err.Error())
        }

        req, err := client.GetGitHubAccessTokenPreparer(ctx, APIVersion, gitHubAccessTokenRequest, clientRequestID)
    if err != nil {
    err = autorest.NewErrorWithError(err, "synapse.WorkspaceGitRepoManagementClient", "GetGitHubAccessToken", nil , "Failure preparing request")
    return
    }
        ...
        err = autorest.NewErrorWithError(err, "synapse.WorkspaceGitRepoManagementClient", "GetGitHubAccessToken", resp, "Failure sending request")
        return
        }

        result, err = client.GetGitHubAccessTokenResponder(resp)
        if err != nil {
        err = autorest.NewErrorWithError(err, "synapse.WorkspaceGitRepoManagementClient", "GetGitHubAccessToken", resp, "Failure responding to request")
        return
        }
